#ifndef DataTreeMonteCarloTrack_H
#define DataTreeMonteCarloTrack_H 1
...
class DataTreeMonteCarloTrack : public TObject
{
    
public:
    const int nSubDetectors = 5;
    const int nIndices = 5;
    
    DataTreeMonteCarloTrack(int idx = 0);
    ~DataTreeMonteCarloTrack();
...
{
    if (idx < Impact_Point[jdx].size()) Impact_Point[0].at(idx) = pX; Impact_Point[1].at(idx) = pY; Impact_Point[2].at(idx) = pZ;
...
void AddDaughterTrack(DataTreeMonteCarloTrack* track){ Daughters.push_back(track); }
DataTreeMonteCarloTrack* GetDaughterTrack(int idx){ if (idx < Daughters.size()) return Daughters.at(i); else std::cout << "Wrong index! " << idx << "/" << Daughters.size() << std::endl; }
...
private:    
    ...
    std::vector <DataTreeMonteCarloTrack*> Daughters;
    ClassDefNV(DataTreeMonteCarloTrack, 1)
};
