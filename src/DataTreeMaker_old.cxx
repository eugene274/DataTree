...
//--------------------------------------------------------------------------------------------------
void DataTreeMaker::ResetSTSMatching()
{
    for (int i=0;i<DTEvent->GetNTracks();i++)
    {
	DTEvent->GetTrack(i)->SetMCTrackId(i);
    }
}
//--------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
bool DataTreeMaker::MCTrackIsMatched(int idx)
{
    bool result = false;
    for (int i=0;i<DTEvent->GetNTracks();i++)
    {
	if (DTEvent->GetTrack(i)->GetMCTrackId() == i) result = true;
    }
    return result;
}
//--------------------------------------------------------------------------------------------------
void DataTreeMaker::Read_MC_Tracks()
{
	...
	if (MCTrackIsMatched(i)) continue;
...
//     cout << "nSTStracks = " << nSTStracks << endl;
    ...
    // 	DTTrack->SetVtxChiSq(DataTreeTrack::kFirstPointTrack,vChiToPrimVtx[i]);
    ...
    // 	DTTrack->SetVtxChiSq(DataTreeTrack::kLastPointTrack,vChiToPrimVtx[i]);
...
//     fitter.Fit(vRTracks, pdg);
	...
	// 		cout << "pT = " << MC_pT << "; eta = " << MC_eta << "; phi = " << MC_phi << endl;
		    ...
		    DTEvent->GetMCTrack(i)->SetPt(MC_pT);
		    DTEvent->GetMCTrack(i)->SetPhi(MC_phi);
		    DTEvent->GetMCTrack(i)->SetEta(MC_eta);
		    DTEvent->GetMCTrack(i)->SetPdgId(type);
		    DTEvent->GetMCTrack(i)->SetMass(mass);
		    DTEvent->GetMCTrack(i)->SetMotherId(motherid);
		}
	    }	
	    //BEGIN vertex point
